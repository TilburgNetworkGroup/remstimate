#' getStats (a modified version of remstats::remstats(). 
#'Documentation is the same except for arguments riskset,actors and types which were removed [discuss with Marlyne])
#' the argument actors and types allow to add actors and types not observed in the sequence. This feature should be added in reh.cpp
#' 
...
getStats <- function(formula, edgelist, dat, directed = TRUE, with_type = FALSE, 
    ...
    if(any(with_typeVar[which(names(effects) == "baseline")])) {
        typesUser <- unique(sort(edgelist$type)) # risksetUser[,3]
    ...
    #evls <- evls[start:stop,] #THIS MUST BE IMPLEMENTED OUT OF THIS FUNCTION
