...
* Since `compute_stats` is not an exported function in `remstats`, _getStats.R_ / _compute_stats.cpp_ / _compute_stats.h_ are temporary functions so as to calculate statistics, run the estimation and compare estimates with `relevent::rem()`.  _getStats.R_ is the alias of `remstats::remstats()` with some modification at the stage of the preprocessing of the network;
