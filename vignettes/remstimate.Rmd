...
The parameters are also available as object within the list, `tie_data$true.pars`. (the event sequence was generated with a package that is not yet available on CRAN)
```{r}
...
tie_bsir <- remstimate::remstimate(reh = tie_reh,
                        ...
                        seed = 23909 # set a seed only for reproducibility purposes
                        )
...
Furthermore, we know that the _statistics_ (name in subscript next to the model parameters $\theta$ and $\beta$) and _true_ parameters' values used in the generation of the event sequence are:
...
The parameters are also available as object within the list, `ao_data$true.pars`. The event sequence was generated with another the package that will be available on CRAN.
```{r}
# setting `ncores` to 1 (the user can change this parameter)
ncores <- 1L
...
```{r}
tie_gd <- remstimate::remstimate(reh = ao_reh,
                        stats =  ao_stats,
                        ncores = ncores,
                        method = "GDADAMAX",
                        epochs = 200L)
...
```{r, out.width="50%", dev=c("jpeg")}
# diagnostics
tie_gd_diagnostics <- diagnostics(object = tie_gd, reh = ao_reh, stats = ao_stats)
# plot
plot(x = tie_gd, reh  = ao_reh, diagnostics = tie_gd_diagnostics)
```
...
library(mvnfast) # loading package for fast simulation from a multivariate Student t distribution
priormvt <- mvnfast::dmvt # defining which distribution we want to use from the 'mvnfast' package
tie_bsir <- remstimate::remstimate(reh = ao_reh,
                        stats =  ao_stats,
                        ncores = ncores,
                        method = "BSIR",
                        nsim = 100L, # 100 draws from the posterior distribution
...
```{r, out.width="50%", dev=c("jpeg")}
# diagnostics
tie_bsir_diagnostics <- diagnostics(object = tie_bsir, reh = ao_reh, stats = ao_stats)
# plot
plot(x = tie_bsir, reh  = ao_reh, diagnostics = tie_bsir_diagnostics)
```
...
```{r}
tie_hmc <- remstimate::remstimate(reh = ao_reh,
                        stats =  ao_stats,
                        method = "HMC",
                        ncores = ncores,
                        nsim = 200L, # 200 draws to generate per each chain
                        nchains = 6L, # 6 chains to generate
                        burnin = 300L, # burnin length is 50
                        thin = 2L, # thinning size set to 2
...
```{r, out.width="50%", dev=c("jpeg")}
# diagnostics
tie_hmc_diagnostics <- diagnostics(object = tie_hmc, reh = ao_reh, stats = ao_stats)
# plot
plot(x = tie_hmc, reh  = ao_reh, diagnostics = tie_hmc_diagnostics)
```
---
