...
# Modeling frameworks {.tabset .tabset-pills}
...
tie_hmc <- remstimate::remstimate(reh = tie_reh,
                        ...
                        thin = 2L, # thinning size set to 2 (the final length of the chains will be 50)
                        seed = 23909 # set a seed only for reproducibility purposes
                        )
...
# plot
plot(x = ao_gd, reh  = ao_reh, diagnostics = ao_gd_diagnostics)
```
...
# plot
plot(x = ao_bsir, reh  = ao_reh, diagnostics = ao_bsir_diagnostics)
```
...
ao_hmc <- remstimate::remstimate(reh = ao_reh,
                        ...
                        nchains = 6L, # 6 chains (each one long 200 draws) to generate
                        burnin = 300L, # burnin length 
                        thin = 2L, # thinning size (this will reduce the final length of each chain)
