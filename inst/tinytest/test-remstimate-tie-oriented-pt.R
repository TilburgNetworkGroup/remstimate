...
# (1) method = "MLE"
expect_silent(remstimate(reh = tie_reh,
                        stats = tie_reh_stats,
                        ncores = 1L,
                        method = "MLE"))
...
tie_stats_baseline <- remstats(reh = tie_reh, tie_effects = ~1)
tie_mle_only_baseline <- remstimate(reh = tie_reh,
...
expect_silent(remstimate(reh = tie_reh,
                        ...
                        WAIC = TRUE))
# WAIC = NULL
expect_error(remstimate(reh = tie_reh,
                        stats = tie_reh_stats,
                        ncores = 1L,
                        WAIC = NULL))
...
expect_silent(remstimate(reh = tie_reh,
                        ...
                        WAIC = TRUE,
                        nsimWAIC = 100))
tie_mle_with_waic <- remstimate(reh = tie_reh,
                        stats = tie_reh_stats,
                        ncores = 1L,
                        WAIC = TRUE,
                        nsimWAIC = 100)
...
tie_reh_ordinal <- remify(edgelist = tie_data$edgelist, model = "tie", ordinal = TRUE)
tie_reh_stats_ordinal <- remstats(reh = tie_reh_ordinal, tie_effects = tie_model)
expect_silent(remstimate(reh = tie_reh_ordinal,
...
tie_reh_omit_test <- remify(edgelist = tie_data$edgelist, model = "tie")
expect_silent(remstimate(reh = tie_reh_omit_test,
                        stats = tie_reh_stats_ordinal,
                        ncores = 1L,
                        WAIC = TRUE,
                        nsimWAIC = 100))
tie_mle_omit_test <- remstimate(reh = tie_reh_omit_test,
                        stats = tie_reh_stats_ordinal,
                        ncores = 1L)
...
expect_silent(remstimate(reh = tie_reh,
                        ...
                        burnin = 5L,
                        seed = set_seed))
tie_hmc <- remstimate(reh = tie_reh,
                        stats = tie_reh_stats,
                        ncores = 1L,
                        method = "HMC",
                        nchains = 3L,
...
tie_reh <- remify(edgelist = tie_data$edgelist, model = "tie") # removing actor "4" from time=120 to time=148
# calculating statistics
tie_reh_stats <- remstats(reh = tie_reh, tie_effects = tie_model)
# (1) method = "MLE"
expect_silent(remstimate(reh = tie_reh,
...
tie_reh <- remify(edgelist = tie_data$edgelist,
                            model = "tie",
                            riskset="active")
# calculating statistics
tie_reh_stats <- remstats(reh = tie_reh, tie_effects = tie_model)
